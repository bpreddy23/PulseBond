...
def register():
    ...
    if os.path.exists(f"profile_data/{name}"):
    ...
    for mood in moods:
        files_for_mood = [f for f in uploaded_files if f.filename.startswith(mood)]
        if not files_for_mood:
            return jsonify({"error": f"No files for mood {mood}"}), 400
        ...
        for file in files_for_mood:
            ...
            pitch, energy = extract_pitch_energy(file_path)
...
def detect():
    ...
    if not os.path.exists(f"profile_data/{name}"):
    ...
    temp_path = "temp/test.wav"
    os.makedirs("temp", exist_ok=True)
    uploaded_file.save(temp_path)

    test_wav = preprocess_wav(temp_path)
    ...
    if similarity < 0.84:
        return jsonify({"error": "Speaker verification failed"}), 403

    pitch, energy = extract_pitch_energy(temp_path)
    distances = {}
    ...
    return jsonify({"mood": mood_detected, "similarity": similarity})
